...
macro_rules! modifier {
    ($(
        ...
        // NOTE (Deslop patch): the target type below was originally written
        // as the projection `<$name as ModifierValue>::Type`. An unnormalized
        // associated-type projection in impl-self position poisons coherence
        // for *downstream* blanket `From<T>` impls (cookie, tauri-utils) on
        // rustc >= 1.88, causing spurious E0119. `target_ty!` expands to the
        // exact same concrete type, but normalized, which fixes it.
        $(#[expect($expect_inner)])?
